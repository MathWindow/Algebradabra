...
class number_list {
public:
	array <uint_32, prime_table_max_size> number{};
	...
	void create() {
		if (!created) {
			...
			uint_32 value_helper = 3;

			bool may_printed = false;

			for (
				uint_16 index_fulling_number = 1; 
				...
				while (!may_printed) {
					...
					for (
						uint_16 index_preview_number = 0;
...
bool is_prime(uint_32 number_input) {
